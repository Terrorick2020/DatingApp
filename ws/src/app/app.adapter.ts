...
export class AppAdapter extends IoAdapter {
  ...
  constructor(
    ...
    if (shardingOptions) {
      this.logger.log(`WebSocket adapter initialized with sharding: instance ${shardingOptions.instanceId} of ${shardingOptions.totalInstances}`);
    }
  }
    ...
    if (this.shardingOptions) {
      ...
      server.use((socket: Socket, next: (err?: Error) => void) => {
        try {
          ...
          // Определяем, какой инстанс должен обрабатывать этого пользователя
          const targetInstance = this.shardingOptions.userShardResolver(telegramId);
          
          // Если это не наш инстанс, отклоняем подключение
          if (targetInstance !== this.shardingOptions.instanceId) {
            this.logger.debug(`Connection from user ${telegramId} rejected - belongs to instance ${targetInstance}`);
            return next(new Error(`Please connect to instance ${targetInstance}`));
          }
