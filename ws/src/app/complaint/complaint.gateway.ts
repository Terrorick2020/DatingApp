...
> {
	...
	async handleCreateComplaint(
		...
		try {
			this.logger.debug(
				`Обработка создания жалобы от ${complaintData.fromUserId} на ${complaintData.reportedUserId}`
			)

			const response = await this.complaintService.createComplaint(
				complaintData
			)
			...
			this.cacheService.delete(
				`user:${complaintData.fromUserId}:complaints:sent`
			)
		} catch (error) {
			this.logger.error(
	...
	async handleUpdateComplaint(
		...
		try {
			this.logger.debug(
				`Обработка обновления жалобы #${complaintData.complaintId} со статусом ${complaintData.status}`
			)

			const response = await this.complaintService.updateComplaint(
				complaintData
			)
	...
	async handleComplaintStatusChanged(
		...
		try {
			this.logger.debug(
				`Получено событие об изменении статуса жалобы #${statusUpdate.id}`
			)
			...
			this.logger.error(
				`Ошибка при обработке изменения статуса жалобы: ${error.message}`,
				error.stack
			)
		}
	}
	...
	async notifyComplaintUpdate(userId: string, updateData: any): Promise<void> {
		try {
			...
			if (this.isUserOnline(userId)) {
				...
				this.logger.debug(
					`Отправлено уведомление об обновлении жалобы пользователю ${userId}`
				)
				...
				this.logger.debug(
					`Пользователь ${userId} оффлайн, уведомление об обновлении жалобы не отправлено`
				)
			}
		} catch (error) {
			this.logger.error(
				`Ошибка при отправке уведомления об обновлении жалобы пользователю ${userId}: ${error.message}`,
				error.stack
			)
		}
	}
	...
	async handleGetUserComplaints(
		...
		try {
			this.logger.debug(
				`Получение жалоб пользователя ${data.userId} типа ${data.type}`
			)
			...
			this.logger.error(
				`Ошибка при получении жалоб: ${error.message}`,
				error.stack
			)
