...
> {
	...
	async handleLikeTrigger(
		...
		this.logger.debug(`Обработка уведомления о лайке для ${triggerDto.telegramId}`);
		this.server
	...
	async notifyUserAboutLike(userId: string, likeData: any): Promise<void> {
		try {
			...
			if (this.isUserOnline(userId)) {
				...
				this.logger.debug(`Отправлено уведомление о лайке пользователю ${userId}`);
			} else {
				this.logger.debug(`Пользователь ${userId} оффлайн, уведомление о лайке не отправлено`);
			}
		} catch (error) {
			this.logger.error(`Ошибка при отправке уведомления о лайке пользователю ${userId}: ${error.message}`, error.stack);
		}
	}
	...
	async handleGetUserLikes(
		@MessageBody() data: { userId: string; type: 'sent' | 'received' | 'matches' }
	): Promise<void> {
		try {
			this.logger.debug(`Получение лайков пользователя ${data.userId} типа ${data.type}`);
			...
			if (cachedLikes) {
				...
				const likes = await this.likeService.getUserLikes(data.userId, data.type);
			...
			this.logger.error(`Ошибка при получении лайков: ${error.message}`, error.stack);
