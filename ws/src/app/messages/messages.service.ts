...
export class MessagesService extends BaseWsService {
	...
	async sendMsg(msgsUpdateMsgDto: MsgsSendMsgDto): Promise<any> {
		try {
			...
			this.sendRequest(MsgsServerMethods.SendMsg, msgsUpdateMsgDto).catch(err =>
				this.logger.error(
					`Error notifying API about new message: ${err.message}`
				)
			)
	...
	async updateInterlocutor(
		...
		try {
				...
				if (userChats) {
					...
					if (targetChatId) {
						...
						this.sendRequest(
							...
							this.logger.error(
								`Error notifying API about interlocutor update: ${err.message}`
							)
						)
			...
			this.logger.error(
				`Error updating interlocutor: ${error.message}`,
				error.stack
			)
			return {
	...
	async updateMsg(msgsSendMsgDto: MsgsSendMsgDto): Promise<any> {
		try {
			...
			return await this.sendRequest(MsgsServerMethods.UpdateMsg, msgsSendMsgDto)
		} catch (error) {
