...
@Injectable()
  ...
  > {
	...
	async handleUpdateInterlocutor(
	  ...
	  try {
		this.logger.debug(`Processing UpdateInterlocutor: ${JSON.stringify(msgsUpdateIntrlocDto)}`);
		...
		if (cachedData) {
		  ...
		  this.messagesService.updateInterlocutor(msgsUpdateIntrlocDto)
			.then(response => {
			...
			.catch(err => this.logger.error(`Failed to update interlocutor: ${err.message}`));
		} else {
		...
		this.logger.error(`Error in handleUpdateInterlocutor: ${error.message}`, error.stack);
	...
	async handleUpdateMsg(@MessageBody() msgsSendMsgDto: MsgsSendMsgDto): Promise<void> {
	  try {
		this.logger.debug(`Processing UpdateMsg: ${JSON.stringify(msgsSendMsgDto)}`);
		...
		if (response && response.chatId) {
		  ...
		  this.redisService.redis.publish('chat:updateMsg', JSON.stringify({
		...
		this.logger.error(`Error in handleUpdateMsg: ${error.message}`, error.stack);
	...
	async handleSendMsg(@MessageBody() msgsUpdateMsgDto: MsgsUpdateMsgDto): Promise<void> {
	  try {
		this.logger.debug(`Processing SendMsg: ${JSON.stringify(msgsUpdateMsgDto)}`);
		...
		if (response && response.chatId) {
		  ...
		  // Публикуем событие для получателя сообщения
		  this.redisService.redis.publish('chat:newMessage', JSON.stringify({
			chatId: response.chatId,
			messageId: response.msgId,
	...
	async sendDirectMessageNotification(userId: string, messageData: any): Promise<void> {
	  try {
		...
		this.logger.error(`Error sending direct message notification: ${error.message}`, error.stack);
	  }
	}
	
	// Получение статуса набора текста (typing)
	@SubscribeMessage('getTypingStatus')
	async getTypingStatus(@MessageBody() data: { userId: string, chatId: string }): Promise<void> {
	  try {
		...
		this.logger.error(`Error getting typing status: ${error.message}`, error.stack);
	  }
	}
	...
	async setTypingStatus(
      ...
      this.logger.error(`Error setting typing status: ${error.message}`, error.stack);
    }
  }
}