...
> {
		...
		try {
			...
			if (cachedChats) {
				...
				this.chatService
					...
					.catch(err =>
						this.logger.error(`Failed to update chats cache: ${err.message}`)
					)
			} else {
		...
		try {
			...
			if (cachedDetails) {
				...
				this.chatService
					...
					.catch(err =>
						this.logger.error(
							`Failed to update chat details cache: ${err.message}`
						)
					)
			} else {
			...
			this.logger.error(
				`Error in getChatDetails: ${error.message}`,
				error.stack
			)
			client.emit('chatDetailsError', {
	...
	async handleGetMessages(
		@MessageBody()
		data: { chatId: string; userId: string; limit?: number; offset?: number },
		@ConnectedSocket() client: Socket
	): Promise<void> {
		try {
			...
			const messageIds = await this.redisService.redis.zrevrange(
				orderKey,
				offset,
				offset + limit - 1
			)
			...
			const messagesData = await this.redisService.redis.hmget(
				messagesKey,
				...messageIds
			)
