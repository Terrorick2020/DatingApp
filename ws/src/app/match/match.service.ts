...
export class MatchService extends BaseWsService {
  ...
  async sendMatchTrigger(triggerDto: MatchTriggerDto): Promise<any> {
    try {
      ...
      const existingNotification = await this.redisService.redis.exists(notificationKey)
      ...
      const response: { message?: string; status?: string } = await this.sendRequest(
      ...
      if (response && (!response.message || response.status !== 'error')) {
        ...
        await this.redisService.redis.hmset(
          ...
          'matchUserName', triggerDto.fromUser.name,
          'matchUserAvatar', triggerDto.fromUser.avatar,
          'createdAt', Date.now().toString(),
          'seen', 'false'
        )
        
        // Установка TTL для данных о матче
        await this.redisService.redis.expire(matchKey, this.MATCH_NOTIFICATION_TTL)
   ...
   */
  async confirmMatchNotification(userId: string, matchUserId: string): Promise<any> {
    try {
      this.logger.debug(
  ...
  async getUserMatches(userId: string): Promise<MatchesResponse> {
    try {
      ...
      if (!matchKeys || matchKeys.length === 0) {
        ...
        const response: MatchesResponse = await this.sendRequest('getUserMatches', { userId })
        ...
        if (response && response.matches && Array.isArray(response.matches)) {
          ...
          for (const match of response.matches) {
            ...
            await this.redisService.redis.hmset(
              ...
              'matchUserName', match.matchUserName || '',
              'matchUserAvatar', match.matchUserAvatar || '',
              'createdAt', match.createdAt?.toString() || Date.now().toString(),
              'seen', match.seen?.toString() || 'true'
            )
