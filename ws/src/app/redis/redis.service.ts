...
export class RedisService {
  ...
  async hset(key: string, ...args: string[]): Promise<number> {
    try {
      ...
      this.logger.error(`Error setting hash field for key ${key}: ${error.message}`);
      return 0;
    }
  }
  ...
  async hget(key: string, field: string): Promise<string | null> {
    try {
      ...
      this.logger.error(`Error getting hash field ${field} for key ${key}: ${error.message}`);
      return null;
    }
  }
  ...
  async hmget(key: string, ...fields: string[]): Promise<(string | null)[]> {
    try {
      ...
      this.logger.error(`Error getting multiple hash fields for key ${key}: ${error.message}`);
  ...
  async hgetall(key: string): Promise<Record<string, string>> {
    try {
      ...
      this.logger.error(`Error getting all hash fields for key ${key}: ${error.message}`);
      return {};
    }
  }
  ...
  async zrevrange(key: string, start: number, stop: number): Promise<string[]> {
    try {
      ...
      this.logger.error(`Error getting reverse range from sorted set ${key}: ${error.message}`);
      return [];
    }
  }
